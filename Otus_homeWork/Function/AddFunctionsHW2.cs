...
namespace Otus_homeWork.Function
{
    internal class AddFunctionsHW2
    {
        ...
        //            if (inputData.Length > UpdateHandler.TaskLengthLimittaskLength)
        //                //throw new TaskLengthLimitException(inputData.Length, Program.TaskLengthLimittaskLength);
        //                throw new CustomException($"Длина задачи '{inputData.Length}' превышает максимально допустимое значение {UpdateHandler.TaskLengthLimittaskLength}");
        ...
        //            var temp = new ToDoItem(UpdateHandler.UserData, inputData);
        ...
        //        if (!allPrint)
        //        {
        //            foreach (var task in UpdateHandler.TaskList)
        //            {
        ...
        //            bool printBorder;
        //            for (int i = 0; i < UpdateHandler.TaskList.Count; i++)
        //            {
        //                printBorder = true;
        //                if (allPrint)
        //                {
        //                    stringToReturn = $"{stringToReturn}{$"| {index++,5} | {UpdateHandler.TaskList[i].GuidId,36} "+
        //                            $"| {UpdateHandler.TaskList[i].CreateAT.ToString(),19} | {UpdateHandler.TaskList[i].State,9} | {UpdateHandler.TaskList[i].TaskName}"
        //                        }";
        //                    stringToReturn = $"{stringToReturn}{HelpFunctions.PrintFrame(' ', leng - UpdateHandler.TaskList[i].TaskName.Length - 1, '|', true)}";
        //                }
        //                else if (UpdateHandler.TaskList[i].State == ToDoItemState.Active)
        //                {
        //                    stringToReturn = $"{stringToReturn}{($"| {index++,5} | {UpdateHandler.TaskList[i].GuidId,36} | {UpdateHandler.TaskList[i].CreateAT.ToString(),19} "+
        //                            $"| {UpdateHandler.TaskList[i].TaskName}"
        //                        )}";
        //                    stringToReturn = $"{stringToReturn}{HelpFunctions.PrintFrame(' ', leng - UpdateHandler.TaskList[i].TaskName.Length - 1, '|', true)}";
        //                }
        //                else
        ...
        //                if (printBorder)
        //                    if (i == UpdateHandler.TaskList.Count - 1)
        //                    {
        //                        stringToReturn = $"{stringToReturn}└";
        //                        if (allPrint)
        //                            stringToReturn = $"{stringToReturn}{HelpFunctions.PrintBorder('─', [6, 37, 20, 10, leng], '┴', '┘')}"; 
